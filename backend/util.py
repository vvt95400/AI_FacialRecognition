...
def load_saved_artifacts():
    ...
    with open(r"C:\New folder\AI_FacialRecognition\backend\artifacts\class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open(r'C:\New folder\AI_FacialRecognition\backend\artifacts\saved_model.pkl', 'rb') as f:
...
def get_b64_test_image_for_virat():
    with open(r"C:\New folder\AI_FacialRecognition\backend\b64.txt") as f:
        return f.read()
